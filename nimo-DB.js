...
function nimoDB() {
  ...
  this.openDatabase = function() {
    ...
    var name = nimoName(name)
     var rst = new Promise(function(r){
       if (false) {
         console.error(`"${decode(name)}" can not be open for it does not exist`)
         r(null)
       }
    var fx = attr.indexedDB.open(name);
     fx.onsuccess = function() {
  ...
  this.deleteAllCreatedDatabase = function() {
   var prm = new Promise(function(r) {
    _this.getAllCreatedDatabase().then(function(e) {
      for (var prop in e) {
        attr.indexedDB.deleteDatabase(prop);
      }
  ...
  this.deleteDatabase = function() {
     var prm = new Promise(function(r) {
    var name = arguments[0];
    var rst = attr.indexedDB.deleteDatabase(name)
     _this.open(_name,def).then(function(e) {
        e.removeItem(name);
        r('done')
      })
   ...
   attr.table = function() {
    ...
    return new Promise(function(r) {attr.indexedDB.open(name,vs).onupgradeneeded = function(e) {
      e = e.target.result;
